    ...
    return (
      <>
        ...
        <input
          type={type}
        style={{
          paddingRight:Array.isArray(icons) && icons?.length ?"48px":"16px"
        }}
          className={cn(
        ...
        />
        {
          Array.isArray(icons) && icons?.map((ic,i)=>(
            <>
            <DynamicIcon className="absolute right-4 top-2" key={i} icon={ic.icon} onClick={ic.onClick}/>
            </>
          ))
        }
        </div>
